...
db = fdb.open(
    cluster_file="/data/v8/fdb/playstation_carnivalfdbserver/000000265/st.cluster"
)
...
def find_timestamp_for_version(tr, version):
    ...
    end_key = fdb.KeySelector.last_less_than(
        timekeeper_prefix_end
    ...
    for k, v in tr.snapshot.get_range(start_key, end_key, limit=1, reverse=False):
        ...
        if version_cur > version:
            print(
                "Version is smaller than anything recorded, estimates might not be accurate. "
            )
    ...
    for k, v in tr.snapshot.get_range(start_key, end_key, limit=1, reverse=True):
        ...
        if version_cur < version:
            print(
                "Version is bigger than anything recorded, estimates might not be accurate. "
            )
            return ts_cur + (version - version_cur) / version_per_sec
        ts_min = ts_cur
...
def getVersion(args):
    print(
        "Executing getVersion with startTime: {}, endTime: {}, mutationTime: {}".format(
            args.startTime, args.endTime, args.mutationTime
        )
    )
...
def main():
    ...
    parser_cmd1 = subparsers.add_parser(
        "getVersion", help="given 3 timestamps, get versions for them"
    )
    ...
    parser_cmd2 = subparsers.add_parser(
        "getTime", help="given a version, get unix and human readable time for it"
    )
